...
const PostsSaved = ({ account }) => {
  ...
  const callDataApi = async (account) => {
    ...
    const results = await axios.get(
      `${process.env.ENDPOINT_SERVER}/api/v1/users/get-all-posts-saved/${account._id}?page=${currentPage.current}&pageSize=${resultsOnPage}`
    );
    return results.data;
  };
  const getListQuery = useQuery(
    ["get-all-posts-saved-detail-user", account],
    () => callDataApi(account),
    {
      cacheTime: Infinity,
