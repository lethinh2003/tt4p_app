...
const Followings = ({ account }) => {
  ...
  const callDataApi = async (account) => {
    ...
    const results = await axios.get(
      `${process.env.ENDPOINT_SERVER}/api/v1/users/get-all-followings/${account._id}?page=${currentPage.current}&pageSize=${resultsOnPage}`
    );
    return results.data;
  };
  const getListQuery = useQuery(
    ["get-all-followings-detail-user", account],
    () => callDataApi(account),
    {
      cacheTime: Infinity,
